...
void Func::genFlow()
{
    for(auto iter = exprs.begin();iter!=exprs.end();++iter)
    {
        Expression* e = *iter;
        iter++;
        e->nexts = list<Expression*>();
        e->prevs = list<Expression*>();
        if(iter != exprs.end() && e->type!=Return)
        e->nexts.push_back(*iter);
        if(e->type==Return)
        e->nexts.push_back(*exprs.rbegin());
        iter--;
        e->use = e->right;
        e->def = e->left;
        if(e->type==Goto)
...
{
    stringstream output;
    output<<name<<" ["<<paramCount<<"] ["<<frameSize<<"]"<<endl;
    for(auto e:exprs)
    {
        switch(e->type)
        {
            case ArithRR:output<<REGNAMEFORVAR(e->left[0])<<" = "<<REGNAMEFORVAR(e->right[0])<<" "<<opstring(e->imm[0])
                            <<" "<<REGNAMEFORVAR(e->right[1])<<endl;break;
            case ArithRRSame:output<<REGNAMEFORVAR(e->left[0])<<" = "<<REGNAMEFORVAR(e->right[0])<<" "<<opstring(e->imm[0])
                            <<" "<<REGNAMEFORVAR(e->right[0])<<endl;break;
            case ArithRI:output<<REGNAMEFORVAR(e->left[0])<<" = "<<REGNAMEFORVAR(e->right[0])<<" "<<opstring(e->imm[1])
                            <<" "<<e->imm[0]<<endl;break;
            case Negative:output<<REGNAMEFORVAR(e->left[0])<<" = - "<<REGNAMEFORVAR(e->right[0]);break;
            case MoveRI: output<<REGNAMEFORVAR(e->left[0])<<" = "<<e->imm[0]<<endl;break;
            case MoveRR: if(REGNAMEFORVAR(e->left[0])!=REGNAMEFORVAR(e->right[0]))
            {
                output<<REGNAMEFORVAR(e->left[0])<<" = "<<REGNAMEFORVAR(e->right[0])<<endl;
            }
            break;
            case ArrayWrite: output<<REGNAMEFORVAR(e->right[0])<<" ["<<e->imm[0]<<"] = "<<REGNAMEFORVAR(e->right[1])<<endl;break;
            case ArrayRead:  output<<REGNAMEFORVAR(e->left[0])<<" = "<<REGNAMEFORVAR(e->right[0])<<" ["<<e->imm[0]<<"]"<<endl;break;
            case IfRR: output<<"if "<<REGNAMEFORVAR(e->right[0])<<" "<<opstring(e->imm[0])<<" "<<REGNAMEFORVAR(e->right[1])
                    <<" goto l"<<e->imm[1]<<endl;break;
            case IfRI: output<<"if "<<REGNAMEFORVAR(e->right[0])<<" "<<opstring(e->imm[0])<<" "<<e->imm[2]
                    <<" goto l"<<e->imm[1]<<endl;break;
            case IfIR: output<<"if "<<e->imm[2]<<" "<<opstring(e->imm[0])<<" "<<REGNAMEFORVAR(e->right[0])
                    <<" goto l"<<e->imm[1]<<endl;break;
            case Goto: output<<"goto l"<<e->imm[0]<<endl;break;
            case FrameLoad: output<<"load "<<e->imm[0]<<" "<<REGNAMEFORVAR(e->left[0])<<endl;break;
            case FrameStore: output<<"store "<<REGNAMEFORVAR(e->right[0])<<" "<<e->imm[0]<<endl;break;
            case GlobalLoad: output<<"load v"<<Analyz::Instance.globalVaribleMap[e->imm[0]]<<" "<<REGNAMEFORVAR(e->left[0])<<endl;break;
            case GlobalLoadAddr: output<<"loadaddr v"<<Analyz::Instance.globalVaribleMap[e->imm[0]]<<" "<<REGNAMEFORVAR(e->left[0])<<endl;break;

            //参数,函数还未处理
            case Call: output<<"call "<<e->funtocall<<endl;break;
            case Return: output<<"return"<<endl;

            case Empty:break;
            case Label:output<<"l"<<e->imm[0]<<":"<<endl;break;
            default: break;
        }
    }
    output<<"end "<<name<<endl;
    return output.str();
}
string Func::Processor()
{
    //InitFunEnv();
    ColorAlgorithmMain();
